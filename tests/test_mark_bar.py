...
def _bars_bases_are_on_x_axis(bars_chart_html: str) -> bool:
    """Verify that the bars are well-placed in the x-axis (the base of the bar is in the x-axis)."""
    soup = BeautifulSoup(bars_chart_html, 'lxml')
...
def _bars_height_does_not_exceed_max_height(bars_chart_html: str, bars_chart: aframexr.Chart) -> bool:
...
class TestMarkBarOK(unittest.TestCase):
    ...
    def test_from_json(self):
        ...
        self.assertTrue(_bars_bases_are_on_x_axis(bars_chart_html))
        self.assertTrue(_bars_height_does_not_exceed_max_height(bars_chart_html, bars_chart))  # type: ignore
    ...
    def test_several_filters(self):
        ...
        for eq in SEVERAL_FILTER_EQUATIONS[1:]:
            bars_chart.transform_filter(eq)
    ...
    def test_aggregate(self):
        ...
        for a in AGGREGATES:
            ...
            bars_chart_2_html = bars_chart_2.to_html()
    ...
    def test_concatenation(self):
        ...
        self.assertTrue(_bars_bases_are_on_x_axis(concatenated_chart_html))
        self.assertTrue(_bars_height_does_not_exceed_max_height(concatenated_chart_html, concatenated_chart))

    def test_environment(self):
        """Scene creation with personalized environment."""
        for e in AVAILABLE_ENVIRONMENTS:
            bars_chart = aframexr.Chart(DATA).mark_bar().encode(x='model', y='sales')
            bars_chart_html = bars_chart.to_html(environment=e)
            self.assertTrue(_bars_bases_are_on_x_axis(bars_chart_html))
            self.assertTrue(_bars_height_does_not_exceed_max_height(bars_chart_html, bars_chart))
