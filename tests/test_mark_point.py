...
class TestMarkPointOK(unittest.TestCase):
    ...
    def test_color(self):
        ...
        for c in SIMPLE_ELEMENTS_COLORS:
            point_chart = aframexr.Chart(DATA).mark_point(color=c).encode(x='model', y='sales')
            point_chart_html = point_chart.to_html()
            self.assertTrue(_every_radius_does_not_exceed_max_radius(point_chart_html, point_chart))
            self.assertTrue(_points_are_inside_chart_volume(point_chart_html))
...
class TestMarkPointError(unittest.TestCase):
    ...
    def test_color_encoding_not_nominal_error(self):
        """Mark point error when color encoding is not nominal."""
        with self.assertRaises(ValueError) as error:
            aframexr.Chart(DATA).mark_point().encode(x='model', y='sales', color='doors').to_html()  # Quantitative
        self.assertEqual(
            str(error.exception),
