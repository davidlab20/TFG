...
class TestMarkLineOK(unittest.TestCase):
    ...
    def test_color(self):
        ...
        for c in SIMPLE_ELEMENTS_COLORS:
            line_chart = aframexr.Chart(DATA).mark_line(color=c).encode(x='model', y='sales')
            line_chart.to_html()
...
class TestMarkLineError(unittest.TestCase):
    ...
    def test_color_encoding_not_nominal_error(self):
        """Line chart error when color encoding is not nominal."""
        with self.assertRaises(ValueError) as error:
            aframexr.Chart(DATA).mark_line().encode(x='model', y='sales', color='doors').to_html()  # Doors quantitative
        self.assertEqual(
            str(error.exception),
